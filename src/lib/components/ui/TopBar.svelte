...
<header class="border-b border-border bg-background sticky top-0 z-[var(--z-modal)]">
	<div
		class="mx-auto max-w-7xl px-6 py-3 grid items-center min-h-[80px] gap-6"
		style="grid-template-columns: 1fr auto 1fr;"
	>
		<!-- LEFT: logo + wordmark -->
		<div class="flex items-center justify-self-start">
			<a href="/" class="flex items-center gap-3" aria-label="Accueil">
				<LogoMark />
				<Wordmark />
			</a>
		</div>

		<!-- CENTER: search (collapses to nothing on /recherche or below lg) -->
		<div class="justify-self-center w-full max-w-[460px]">
			{#if !onRecherche}
				<form
		...
		<div class="flex items-center gap-6 justify-self-end">
			<nav class="hidden md:flex items-center gap-6 font-ui text-sm font-semibold">
				<CatechismDropdown />
