	...
	$effect(() => {
		...
		(async () => {
			...
			try {
				...
				// The same failure behaviour as the verse Liturgie tab: never leave
				// the previous context's questions on screen under a new heading,
				// and never claim the Compendium has none when a fetch simply
				// failed.
				if (mine !== request) return;
				hits = [];
				failed = true;
...
{:else if failed}
	<p class="text-muted italic font-ui text-sm">
		Les questions du Compendium n'ont pas pu être chargées. Rouvrez l'onglet pour réessayer.
	</p>
