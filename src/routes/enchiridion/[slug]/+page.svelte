<script lang="ts">
	...
	function renderEntry(html: string): string {
		...
		// truth: scripts/prepare/enchiridion/extract.py). Render-time only
...
<svelte:head>
	<title>{unit.title} · Enchiridion Symbolorum</title>
	<meta
		name="description"
		content={`${unit.title} · ${unit.entries.length} entrées de l'Enchiridion Symbolorum (DH ${unit.entries[0]?.n}–${unit.entries[unit.entries.length - 1]?.n}).`}
	/>
</svelte:head>
...
<main class="dz-unit" use:scrollSpy>
	<header class="head">
		<BreadcrumbRail
			crumbs={[
				{ href: '/enchiridion', title: 'Enchiridion Symbolorum' },
				{ href: `/enchiridion/${unit.slug}`, title: unit.title }
			]}
		/>
	...
	<nav class="pager" aria-label="Sections adjacentes dans la même partie">
		{#if unit.prev}
			<NavCard
				href="/enchiridion/{unit.prev.slug}"
		...
		{#if unit.next}
			<NavCard
				href="/enchiridion/{unit.next.slug}"
