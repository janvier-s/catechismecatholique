...
<svelte:head>
	<title>Sommaire · Enchiridion Symbolorum</title>
	<meta
		name="description"
		content="Sommaire complet de l'Enchiridion Symbolorum (Denzinger) : toutes les sections classées par parties."
	/>
</svelte:head>

<main class="toc">
	<header class="toc-head">
		<p class="eyebrow">Enchiridion Symbolorum · Denzinger</p>
		...
		<div class="toc-actions">
			<a class="index-link" href="/enchiridion">← Retour à l'accueil</a>
		</div>
	</header>
				...
				<div class="units">
					{#each part.units as unit (unit.slug)}
						...
						<div class="unit-block">
							...
							{/if}
							<a class="row" href="/enchiridion/{unit.slug}">
							...
							{#if unit.sections && unit.sections.length > 0}
								<div class="sections">
									{#each unit.sections as sec (sec.anchor)}
										{#if sec.title}
											<a class="row row-section" href="/enchiridion/{unit.slug}#{sec.anchor}">
												<span class="row-label">
										...
										{#if sec.chapters && sec.chapters.length > 0}
											<div class="chapters">
												{#each sec.chapters as ch (ch.anchor)}
													<a class="row row-chapter" href="/enchiridion/{unit.slug}#{ch.anchor}">
														<span class="row-label">
