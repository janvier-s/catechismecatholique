...
**Reading-page link:**
- `src/lib/components/bible/BibleReader.svelte` — add a small button at the top: *"Voir la concordance"* → `/bible/{slug}/{ch}/concordance`. Hidden when no concordance data for this chapter (`manifest`-based check).
...
export function buildConcordancePericopes(
  ...
  for (const html of htmlFiles) {
    ...
    for (const entry of file.entries) {
      ...
      for (let ch = startCh; ch <= endCh; ch++) {
        ...
        if (!book) { book = new Map(); byBook.set(usfx, book); }
        let draft = book.get(ch);
        if (!draft) { draft = { pericopes: [], verseSet: new Map() }; book.set(ch, draft); }
    ...
    for (const [ch, draft] of chapters) {
      ...
      draft.pericopes.sort(
        (a, b) =>
          ...
          (b.endVerse - b.startVerse) - (a.endVerse - a.startVerse) // broader first when ties
      );
  ...
  for (const [pNum, entries] of byParagraph) {
    entries.sort((a, b) =>
      a.slug.localeCompare(b.slug) || a.chapter - b.chapter || a.verseRef.localeCompare(b.verseRef)
    );
...
describe('buildConcordancePericopes', () => {
  ...
  it('sorts pericopes by startVerse asc, broader first on ties', () => {
    ...
    expect(ps.map(p => p.verseRef)).toEqual([
      'Genèse 3:1-24', 'Genèse 3:1-7', 'Genèse 3:15'
    ]);
  });
  ...
  it('builds by-paragraph inverse', () => {
    ...
    expect(r.byParagraph['390']).toEqual([
      { slug: 'genese', usfx: 'GEN', bookFrenchName: 'Genèse', chapter: 3,
        verseRef: 'Genèse 3:1-24', pericopeTitle: 'La faute et le châtiment' }
    ]);
...
logStep('building concordance pericopes');
{
  ...
  const sourceDir = process.env.DIDACHE_SOURCE_DIR
    ?? join(ROOT, '..', 'DOCTRINA', 'sources', 'didache');
  ...
  const { byBook, byParagraph, manifest, stats } = buildConcordancePericopes(
    htmlFiles, ncl, knownParas, BOOKS, nclSections
  );
  ...
  try { unlinkSync(join(OUT, 'ccc/concordance-verse-index.json')); } catch {}

  if (stats.unknownBooks.length > 0) console.warn('  unknown books:', stats.unknownBooks.join(', '));
  if (stats.unknownParagraphs.length > 0) console.warn(`  ${stats.unknownParagraphs.length} unknown CCC paragraphs (dropped)`);
  if (stats.unparseableRanges.length > 0) console.warn(`  ${stats.unparseableRanges.length} unparseable ranges`);
  if (stats.booksWithZeroEntries.length > 0) console.warn(`  ${stats.booksWithZeroEntries.length} books with zero entries: ${stats.booksWithZeroEntries.join(', ')}`);
  if (stats.pericopesWithoutTitle > 0) console.warn(`  ${stats.pericopesWithoutTitle} pericopes without NCL title (kept titleless)`);
    ...
    <div class="space-y-[2px]">
      {#each verses as verse (verse.v)}
        ...
        >
          ...
          <span class="flex-1 font-body text-[15px] leading-relaxed text-foreground"
...
{#if entries.length === 0}
  <p class="font-ui text-xs italic text-subtle">
    Aucun renvoi pour ce paragraphe.
  </p>
{:else}
  <ul class="space-y-2">
...
test('clicking a pericope header on the left selects the matching pericope on the right', async ({ page }) => {
  await page.goto('/bible/genese/3/concordance');
